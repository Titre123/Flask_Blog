...
def user(username):
    ...
    posts = Post.query.filter_by(author = user).order_by(Post.date_posted.desc()).paginate(per_page = 2)
    return render_template("user_post.html", posts=posts, user = user)


def send_reset_email(user):
    token = user.get_reset_token()
    msg = Message('Password Reset Request', sender='triumphanttaiwo@gmail.com', recipients=[user.email])
    msg.body = f'''To reset your password, visit the following link: 
    {url_for('reset_token', token=token, _external=True)}
    
    If you didnt make this request then simply ignore this email and no changes will be made
    '''
